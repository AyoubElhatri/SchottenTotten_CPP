...
void Human::playCard() {
    ...
    // Si c'est la carte Recruiter, on la retire tout de suite, on lance l'effet, puis on interrompt le tour
    if (selectedCard->getName() == "Recruiter") {
        // Retirer Recruiter de la main et mettre dans la défausse
        getPlayerDeck().moveCard(cardIndex, board.getDiscardedCards());

        // Lancer l'effet du Recruiter
        dynamic_cast<RusesCards*>(selectedCard)->getEvent(nullptr);

        // Interrompt le tour, passe au joueur suivant
        throw std::runtime_error("RECRUITER_END_TURN");
    }
    ...
    // Piocher une carte si disponible (sauf si Recruiter joué, on ne vient pas ici)
    bool canDrawClan = board.getRemainingClanCards().getSize() > 0;
    bool canDrawTactical = board.getRemainingTacticalCards().getSize() > 0;
    ...
    do {
        ...
        if (choice == "1") {
            try {
                ...
                if (std::string(e.what()) == "RECRUITER_END_TURN") {
                    DisplayManager::getInstance()->output("Effet de Recruiter : fin immédiate du tour.\n");
                    return;
                } else {
                    throw;
                }
